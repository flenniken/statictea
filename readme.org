...
When you call a function variable without a prefix, statictea for
it in the function dictionary. 

Otherwise when you use a variable without a prefix, statictea
looks for it in the local dictionary.
...
All the functions exist in the f dictionary by name. Each
...
optional applications used to build and test.

The statictea source folder is shared with the docker container,
so you can use your own editor and applications on the host as
well.
...
- [[#readJson][readJson]] -- Read the JSON string and return it as a variable.
