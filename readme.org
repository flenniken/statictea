...
end lines of the replacement block.  You end the block command
...
You primarily use the system variables and functions on a command
line to control what the command does.
...
match.  You do this with the t.else variable as the case value.
...
Normally you can pick one of the three cases you want.

template:

#+BEGIN_SRC
<!--$ nextline sold = t.cmp(earlgrey, oolong); \-->
<!--$ : bestSelling = t.case(sold, 1, earlgreyName, t.else, oolongName) -->
...
condition, the second is the 1 case and the third is the 0
...
default values. The following example shows the default values of
so of the system variables.
...
#+BEGIN_SRC
<!--$ block t.result="stderr"; t.list = t.if(t.exists(admin), "", "showBlock") \-->
