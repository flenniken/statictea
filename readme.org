...
contains instructions for creating files, running them and for
comparing them to determine whether the test passed.
...
all the stf files. See the runner help message for more
information about stf files.

There are nimble tasks to run all the tests, to run one test, to
make a new test and to list them.  See the nimble section above.
