...
a nextline, block and replace command. You can continue the
continue command too.
...
t.repeat and t.row. Flow of control is done with map, filter,
reduce and other functional language techniques. Output control
is done with t.output.
...
lines. At the end of the line is an optional slash continuation
...
slash character. The following two nextline commands are
equivalent:
...
prefixes. All variables are immutable.  If you try to change a
variable, a warning is generated and the statement is skipped.

Here are the prefixes and sections for each one.
...
function.  You create them in the json files, with the dict
function, or from other dictionaries using map or filter
functions.
...
If a key has valid variable syntax, you can access the it as a
variable, for the json example above, you can access as:
...
get(t.server, "a") => 1
get(t.server, "b") => 2
get(t.server, "d") => dict("x", 100, "y", 200)
...
json files or with the list function.
...
the postfix with a comma and the postfix is optional.

todo: change prefix postfix separator from space to comma.
...
- [[#dictfun][dict()]] -- create a dictionary
- [[#dup][dup()]] -- duplicate string
...
- [[#listfun][list()]] -- create a list
- [[#replace][replace()]] -- replace range with a string
- [[#replaceRe][replaceRe()]] -- replace regular expressions patterns in a string
- [[#substr][substr()]] -- extract a substring from a string by indexes

Testing:

- [[docs/runFunction.md#user-content-a8][cmp()]] -- spaceship compare function <=>
- [[docs/runFunction.md#user-content-a11][get()]] -- get an element from a list or dictionary
- [[docs/runFunction.md#user-content-a12][if()]] -- handles two conditions
...
#+BEGIN_SRC
The abbr. for Darjeeling is Darj.
#+END_SRC
...
its index position when found. The first parameter is the string
and the second is the substring. The third optional parameter is
returned when the substring is not found.  A warning is generated
when the substring is missing and no third parameter.  Added in
version 0.1.0.
...
Tetley name has {length} characters.
...
starting index position and the third is the length of the
substring and the forth is the string to replace.

You can use replace to insert or append to a string.

#+BEGIN_SRC
replace("Earl Grey", 5, 4, "of Sandwich")
  => "Earl of Sandwich"
replace("abc", 3, 0, "123") => "abc123"
#+END_SRC
...
The basic case takes three parameters, the string to work on, the
regular expression pattern, and the replacement string.

In general you can have multiple sets of patterns and
replacements. You add each pair of parameters at the end.  If the
second parameter is a list, the patterns and replacements come
from it.
...
The substr function extracts a substring from a string. It takes
two or three parameters, the string, the substring's starting
index and the end position (one past the end).  The third
