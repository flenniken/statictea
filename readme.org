...
A statement assigns a variable a value using the equal sign. The
varable being assigned is on the left hand side and on the right
hand side is either another variable or a function.  Here are
examples of both:
...
- t.replaceContent -- content for the replace command
