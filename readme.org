...
- [[#return][return]] -- A return in a statement causes the command to stop processing the statement and following statements.
...
A return in a statement causes the command to stop processing the statement and following statements. It returns the value passed in.

In a user defined function, the return function completes the
function and returns the value of the function.

In a template command a return controls the replacement block
looping. There are two variations, "stop" and "skip":
