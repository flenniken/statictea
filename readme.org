...
A dot name is a fully qualified variable name.  You can string up
to five variable names together to make a dot name. For example:

#+BEGIN_SRC
s.tea.black.earlgrey.vanilla
#+END_SRC

A dot name refers to a nested variable. For example, you
reference the server json variables with the letter s,
i.e. s.name, s.address, etc.
...
:PROPERTIES:
   :CUSTOM_ID: conditional-evaluation
   :END:

The if0 and if1 funtions evaluate the matching parameter and the
other parameter is skipped.  This is important when you use the
warn or return functions to control the statement flow.

The other functions evaluate all their parameters before passing
...
The next example has a warn call in it.  If all the parameters
...
warn function ends the current statement and the return function
...
- [[#if1][if1()]] -- If the condition is 1, return the second parameter, else return the third.
