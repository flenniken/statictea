...
#+BEGIN_SRC
<!--$ nextline -->
Drink {v.drink} -- {v.drinkType} is my favorite.
#+END_SRC
...
<!--$ replace t.content=s.header -->
<!DOCTYPE html>
<html lang="{v.languageCode}" dir="{v.languageDirection}">
<head>
<meta charset="UTF-8"/>
<title>{v.title}</title>
<--$ endreplace -->
...
#+BEGIN_SRC
<!--$ nextline message=t.if(v.admin, 'Earl Grey', 'Jasmine') -->
<h2>{v.message}</h2>
#+END_SRC
...
converted to a string when passed to a function takeing a sring.
A float has a decial point.
...
with the "s." prefix, and the v prefix for the the server
variables, no prefix for the local variables.

Variables defined in the template are local to the block where
they are defined. Local variables take precedence over the
json variables.

Variables defined in templates can add t, v, s vars to the local
vars which overrides the others.  You can only override some t
vars.
...
<h4>Tea Companies</h3>
<select>
<!--$ nextline t.list=v.companyList; \-->
<!--$ : current=t.if(v.selected, ' selected="selected"', "") -->
  <option{v.current}>{v.company}</option>
</select>
#+END_SRC
...
<--$ : 'Darjeeling', "Darj",  \-->
<--$ : "Earl Gray", "EG") -->
The abbreviation for {v.tea} is {v.abbr}.
#+END_SRC
...
#+BEGIN_SRC
<!--$ nextline sold = t.cmp(v.earlgrey, v.oolong); \-->
<!--$ : bestSelling = t.case(v.sold, v.earlgreyName, 0, v.oolongName) -->
The best selling tea this week is {v.bestSelling}.
#+END_SRC
...
#+BEGIN_SRC
<--$ nextline -->
Kathleen spent ${v.cost:.2f} on tea for Steve's birthday.
#+END_SRC
...
#+BEGIN_SRC
<!--$ nextline t.list=v.companies \-->
<!--$ : id = row(0); num = run(1) -->
  <li id="r{id}>{num}. {v.teaCompany}</li>
#+END_SRC
...
#+BEGIN_SRC
<!--$ nextline -->
You're a {v.webmaster}, I'm a {v.teaMaster}!
#+END_SRC
...
#+BEGIN_SRC
template.html(2): w1: Unknown server variable: v.teaMaster
#+END_SRC

result:

#+BEGIN_SRC
You're a html wizard, I'm a {v.teaMaster}!
#+END_SRC
...
warnings. Statictea.log is created in the current folder.  Log
information is appended to the file and it grows without
bounds. Make sure to setup log rotation.
...
endings are supported on all platforms: LF, CR LF, or CR. Any
style is read and the platform style is written.  The template
syntax only uses ascii.
...
<!--$ nextline                                   \-->
<!--$ : cmp = t.cmp(len(v.name), 10);            \-->
<!--$ : name = t.case(cmd, v.name                \-->
<!--$ : 1, t.concat(substr(v.name, 0, 7), "...")) -->
#+END_SRC
