...
2. format the json as markdown using the
   [[templates/nimModuleIndex.md][nimModuleIndex.md]] statictea template. Click the link then "View Raw".
...
2. format the json as markdown using the [[templates/nimModule.md][nimModule.md]] and the [[templates/nimModule.tea][nimModule.tea]]
   statictea files.
...
- [[#listLoop][listLoop]] -- Create a new list from a list and a callback function.
...
- [[#return][return]] -- Return is a special function that returns the value passed in and has side effects.
...
- [[#sub][sub]] -- Sub two floats.
- [[#sub][sub]] -- Subtract two integers.
