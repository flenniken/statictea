...
its position when found. The first parameter is the string and the
second is the substring. The third optional parameter is returned
when the substring is not found.  A warning is generated when the
...
starting position and the third is the length of the
substring. The first position is 0.
