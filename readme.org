...
Local variables and writable tea variables cleared after
...
When you set t.repeat to 0, it short circuits the command and
exits.  The commands' statements following are not run.
...
The readonly t.version variable contains the current version
...
- [[#if0][if0]] -- If the condition is 0, return the second parameter, else return
the third.
- [[#if1][if1]] -- If the condition is 1, return the second parameter, else return
the third.
