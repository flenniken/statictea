...
In the following example we define a function called l.cmp that
takes two parameters, a string called numStr and an integer
called num2 and it returns an integer.

#+BEGIN_SRC
l.cmp = func(numStr: string, num2: int) int
  ## Compare a number string
  ## with a number, return 1, 0, -1.
  num1 = int(numStr)
...
Here is how you call the example l.cmp function:
...
The variable name you assign to the function becomes the
function name which is stored internally in the func
variable. You see the name when you get the function details,
otherwise it's not used.

**Function Signature:**

A function signature tells the parameter names, their types and
the return type.

Functions can have the same name as long as their signatures are
different. Signatures are the same when they resolve to the same
set of parameter types.  A signature using “all” or “optional”
may resolve to multiple even when it’s not an exact match.
...
function; you can use returns in IF statements and they don’t end
the function.
...
- if - conditional evaluates its arguments and it has a bare no
  return value option, see see [[#if-functions][If Functions]]
- and — conditional evaluates its arguments
- or — conditional evaluates its arguments
