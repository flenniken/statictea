...
A json dictionary item key becomes the variable name its value
...
blocks using a t.list. The function takes a parameter which
...
The the server json variables get added to the t.server
