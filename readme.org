...
The jsondocraw command replaces the descriptions in the json with
the text from the source file as is. The nimble *jsondoc* command
creates the jsondocraw exe.  jsondocraw calls nim's jsondoc
command then patches the descriptions.
...
- *docmix*:  Create markdown docs index (docs/index.md).
...
#+BEGIN_SRC
tea> pf f
add         exists           if0       lt            slice
anchors     find             if        lte           sort
and         float            int       markdownLite  startsWith
bool        format           join      ne            string
case        func             joinPath  not           sub
cmp         functionDetails  keys      or            type
cmpVersion  get              len       path          values
concat      gt               list      readJson      warn
dict        gte              listLoop  replace
dup         highlight        log       replaceRe
eq          html             lower     return
#+END_SRC
