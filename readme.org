...
Integers are 64 bit signed numbers.  When you pass an integer to
...
You can fetch items from lists defined in json files with
t.get. You define the lists in the json files.
...
one for the shared json.  You access the server json variables
with the "s." prefix and the shared json variables with the "h."
prefix, no prefix for the local variables.
...
works and they are prefixed with "t."
...
You use functions on a command line to assign variables.

The following example assigns the greeting variable a personalized
greeting, then it uses the variable in the replacement block.
...
lists. The function takes a parameter which specifies the
starting point. You can use it in replacement blocks.
...
of which prefixes get used and have specify meaning.
...
option. You reference them with the "h." namespace prefix. The
