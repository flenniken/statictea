...
You can also append to a dictionary using bracket notation on the
left hand side. The index is a variable name or a literal
string. For example:
...
looping and as callbacks for building lists and dictionaries, see
the listLoop function.
