...
The continuation command allows you to continue adding statements
...
#+BEGIN_SRC
statictea --server server.json --shared shared.json \
  --template template.html --result result.html
#+END_SRC
...
- update - update the template replace blocks. See the [[#commands][replace command]].
