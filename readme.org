...
The next lines are for the doc comment followed by the statement
lines.  A return statement ends the function definition.

In the following example we define a function called strNumCmp
and assigns the result to the mycmp function variable. The
...
signature function name is not used except in the function
details.

You can specify an optional last parameter.  The built in get
