...
A command marks replacement content in the template and it
provides a place for your code statements.
...
You can assign a new variable to a list or dictionary but you
cannot change an existing variable.

You append to a list by assigning a value to it. This allows
creation of dynamic lists at runtime. For example, a teas
variable is created with "black" and "green" values:
...
function. The dictionary order is determined by the order of
items when it is created.
...
allowed in strings. The maximum command line length is 1024
...
because it is formatted as gitlab markdown. Statictea created the
...
- [[#githubAnchor][githubAnchor()]] -- Create Github markdown anchor names given a list of heading
names.
- [[#githubAnchor][githubAnchor()]] -- Create a Github markdown anchor name given a heading name.
- [[#if][if()]] -- If the condition is 1 return one value, else return another value.
...
- [[#len][len()]] -- Length of a string in characters, not bytes.
