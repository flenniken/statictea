...
- t. -- tea variables / system variables
- s. -- server variables
- h. -- shared variables
...
The built in system variables, called tea variables, are prefixed
with "t." and they control how the replacement block works.
...
#+BEGIN_SRC
<!--$ nextline message=if(s.admin, 'Earl Grey', 'Jasmine') -->
<h2>{message}</h2>
#+END_SRC
...
The concat function takes any number of string parameters.  The
following example the concat function is passed two parameters to
concatenate and it assigns the result to the greeting variable.
...
#+BEGIN_SRC
<!--$ block \-->
<!--$ : sold = cmp(s.earlgrey, s.oolong); \-->
<!--$ : bestSelling = case(sold, \-->
...
'' <= concat()
'Tea' <= concat("Tea")
'TeaTime' <= concat("Tea", "Time")
'Tea Time' <= concat("Tea", " ",  "Time")
#+END_SRC
