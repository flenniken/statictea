...
from left to right.  You do not have to specify a prefix for
local variables but you can use l. They are stored in the l
dictionary. The local variables are cleared and recalculated for
each repeated block.
...
You use the dict function to create a literal dictionary and the
list function to create a literal list.
...
Static returns success, return code 0, when there are no
warning messages, else it returns 1.
...
You can read module documentation in Gitlab because they are
formatted as gitlab markdown files. Statictea created the files
from the embedded doc comments.

- [[docs/index.md][Module Index]] --- Index to the nim module documents.

The statictea.nimble docs task builds the markdown files by
running the following process:

- extract the doc comments as json using nim's jsondoc comment
- format the json as markdown using a statictea template

** Unformatted Json

Nim supports restructured formatted doc comments. The jsondoc
produces html formatted json data. Our desired format is Gitlab
markdown so having html presentation data in the json is a
problem. It was discovered that single paragraphs are left
unformatted, which is the basis for a workaround. You make all
the documentation look like one paragraph. This is done by
inserting some symbols in strategic places.
...
json        Display nim's doc json for a source file; specify part of name.
jsonix      Display the module index json for the source files.
docsix      Display the doc comment index to the source files.
...
This section was extracted from the nim source code formatted for
this readme using the StaticTea template
[[templates/readmeFuncSection.org][readmeFuncSection.org]]. See the readmefun nimble task.
...
- [[#values][values()]] -- Create a list of the values in a dictionary.
