...
A error reading a statement stops processing the code file, for
...
looping (listLoop) and as callbacks for building lists and
dictionaries.
...
#+BEGIN_SRC
u.mycmp(numStr1: string, numStr2: string) int
#+END_SRC
...
templates. The nimble *docsix2* task builds the module index and
the *docs2* task builds one or more HTML modules docs.
...
- *docsall*:  Create all the docs, docsix, teafuncs, stfix, dot, dot2.
- *release*:  Run tests and update docs; test, other, docsall.
- *b*:  Build the statictea release exe (bin/x/statictea).
- *docsix*:  Create markdown docs index (docs/index.md).
- *docs*:  Create one or more markdown docs; specify part of the name.
...
- *dot*:  Create source module dependency graph (docs/staticteadep.svg).
- *dot2*:  Create system modules dependency graph (docs/staticteadep2.svg).
...
files. The runhelp nimble task shows the stf help with glow.
