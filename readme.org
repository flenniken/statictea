...
minuses and underscores limited to a total of 64 ASCII
characters. Some single letters are reserved for important
...
Variables you add to the g dictionary are global. They’re visible
to all commands.
...
the “a” func variable is defined to be equal to the second built
in cmp function.
...
You pass two or three arguments to the if/if0 functions and they
are special in a few ways:

- they conditionally evaluate their arguments
- the 2 parameter case assigns nothing when the condition is false
- the 2 parameter case can be used in a statement without an
  assignment when using the return or warn function

Here are examples of the cases:
...
if(cond, return("stop"))
if(cond, warn("out of tea!"))
#+END_SRC

**Conditional Evaluate**

The if/if0 function evaluates its condition argument first to
determine which argument to evaluate next and the non-matching
argument is skipped. Normally functions evaluate all their
arguments before passing them to the function.

In the following example doThis is executed and doOther is
skipped.

#+BEGIN_SRC
a = if(true, doThis(), doOther())
#+END_SRC
...
**False Two Parameter Case**

The 2 parameter case assigns nothing when the condition is
false. In the next example, the variable a and b are not created.
...
**No Assignment Two Parameter Case**

The 2 parameter case can be used in a statement without an
assignment when using the return or warn function. This is the
only place the return function can be used as an argument (you
can use warn anywhere).

The return function has a side effect of controlling the
statement flow.

~~~
if(true, return("stop"))
if(true, warn("Buy more tea!"))
~~~
...
A stf file contains the test which the runner executes to
...
testfiles folder contains [[testfiles/readme.md][StaticTea Stf Files]].
...
- [[#functionDetails][functionDetails]] -- Return the function details.
...
- [[#listLoop][listLoop]] -- You use the listLoop function to make a new list.
...
- [[#return][return]] -- A return function returns the value passed in and it has side effects depending on where it is used.
...
- [[#ctionDetails][ctionDetails]] -- Convert the function spec to a dictionary value.
