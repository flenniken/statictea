...
value, a function with its name and the other variable types get
replaced with their json equivalent.
...
All the functions exist in the f dictionary by name. Their value
is a list of func values with the same name.
...
There are four cmp functions so the f dictionary value for it is
a list of four function variables:

#+BEGIN_SRC
cmp => [func, func, func, func]
#+END_SRC
...
- [[#cConcat][cConcat]] -- Concatentate two strings.
