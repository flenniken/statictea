...
Each string variable gets replaced with its value. The other
variable types get replaced with their json equivalent.
...
passed. The stf file contains instructions for creating files,
running files and comparing files. See the runner help message
for more information about stf files.

The testfiles folder contains statictea stf files. The stf files
are designed to look good in markdown readers.
...
- [[#slice][slice()]] -- Extract a substring from a string by its position.
