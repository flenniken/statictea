...
a nextline, block and replace commands.
...
You format server content with variables you create in
statements.
...
newlines and they may flow through the following lines. At the
end of the line is an optional plus continuation character then
the optional postfix then the end of line, either \r\n or \n.
...
A variable begins with an ascii letter followed by letters,
digits or underscores.  They are limited to xxx characters.  You
can use single letters a, b, c, d, e and v, w, x, y, z. The other
single letters are reserved.
...
You enter unicode values with \u and four hex digits or, for
values greater the U-FFFF, you use two pairs.  The two pairs are
...
The dict type is an ordered key value store with
fast lookup. It maps a string key to a value which can be any
type. The dict is ordered by insertion order.

You create a dict in a json file or with the dict function. You
append to them when you create new variables.
...
pairs. StaticTea preserves the order of the elements.
...
contains instrutions for creating test files, running them and
