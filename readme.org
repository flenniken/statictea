...
Example html hello world template:
...
- : - [[#continue-command][continue command]], used for continue a command line
...
A json dictionary key becomes the variable's name and its value
becomes the variable's value.

Internally one dictionary exists for the server json, one for the
...
** System Variables
:PROPERTIES:
   :CUSTOM_ID: system-variables
   :END:

The built in system variables, prefixed with "t.", control how
the replacement block works.

- [[#t-content][t.content]] - content of the replace block
- [[#t-repeat][t.repeat]] - controls how many times the block repeats
- [[#t-local][t.local]] - dictionary containing the current block's local variables.
- [[#t-maxlines][t.maxlines]] - maximum lines before endblock or endreplace
- [[#t-server][t.server]] - dictionary containing the server variables
- [[#t-shared][t.shared]] - dictionary containing the shared variables
...
The variable you assign to t.repeat must be a number or a list
containing dictionaries. Each dictionary provides another source
of local variables, the first dictionary goes with the first
row, the sceond goes with the secon row and so on. All the
block's local variables get recalculated for each row.
...
:PROPERTIES:
:CUSTOM_ID: json-files-and-variables
:END:
...
- [[#t-case][t.case()]] -- generalized if function
- [[#t-cmp][t.cmp()]] -- spaceship compare function <=>
- [[#t-concat][t.concat()]] -- concatenate strings
- [[#t-copy][t.copy()]] -- copy variables to the local dictionary
- [[#t-currency][t.currency()]] -- format currency
- [[#t-exists][t.exists()]] -- whether a variables exists
- [[#t-find][t.find()]] -- find a substring in a string
- [[#t-format][t.format()]] -- format a string
- [[#t-if][t.if()]] -- if function
- [[#t-len][t.len()]] -- length of string
- [[#t-quotehtml][t.quoteHtml]] -- replace html special characters
- [[#t-row][t.row()]] -- special function that returns the current row
- [[#t-sizes][t.sizes()]] -- format bytes counts, KB, MB, GB, etc.
- [[#t-substr][t.substr()]] -- extract a substring from a string by indexes
- [[#t-time][t.time()]] -- format the date and time.
- [[#t-version][t.version()]] -- the current version and version checker
- [[#t-warn][t.warn()]] -- format a warning message with file and line number.
...
** t.currency()
:PROPERTIES:
:CUSTOM_ID: t-currency
:END:

The t.currency function formats numbers as currency.
...
** t.quoteHtml()
:PROPERTIES:
:CUSTOM_ID: t-quoteHtml
:END:

The t.quoteHtml replaces special html characters with equivalents.
...
The t.sizes function formats number of bytes as KB, MB, GB, etc.
...
The t.time function formats data and time values.
...
The t.warn function formats a warning message with file and line
number and writes it to standard out.
