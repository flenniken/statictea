...
Nim Compiler Version 2.2.0 [Linux: arm64]
...
Note: Mac development is on pause until it is easier to install
nim there. Currently choosenim doesn't work on an ARM Mac. The
following instructions are for an Intel Mac.
